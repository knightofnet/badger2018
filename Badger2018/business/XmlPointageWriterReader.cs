    ...
    class XmlPointageWriterReader
    {
        ...
        private void SaveBadgeages(PointageElt pElt)
        {
            ...
            if (_pWinRef.EtatBadger >= 0)
            {
                ...
                DbbAccessManager.Instance.AddBadgeageForToday(0, _pWinRef.Times.PlageTravMatin.Start.TimeOfDay);
            }
            if (_pWinRef.EtatBadger >= 1)
            {
                pElt.B1 = _pWinRef.Times.PlageTravMatin.EndOrDft.ToString();
                DbbAccessManager.Instance.AddBadgeageForToday(1, _pWinRef.Times.PlageTravMatin.EndOrDft.TimeOfDay);
            }
            if (_pWinRef.EtatBadger >= 2)
            {
                pElt.B2 = _pWinRef.Times.PlageTravAprem.Start.ToString();
                DbbAccessManager.Instance.AddBadgeageForToday(2, _pWinRef.Times.PlageTravAprem.Start.TimeOfDay);
            }
            if (_pWinRef.EtatBadger >= 3)
            {
                pElt.B3 = _pWinRef.Times.PlageTravAprem.EndOrDft.ToString();
                DbbAccessManager.Instance.AddBadgeageForToday(3, _pWinRef.Times.PlageTravAprem.EndOrDft.TimeOfDay);
            }
